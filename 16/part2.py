...
for i in range(len(data)):
    for j in range(len(data[i])):
        for direction in directions:
            shortest_distance_to[((j, i), direction)] = inf
    ...
    for current_path in [path for path in paths if not path.is_terminated]:
        ...
        if len(possible_steps) == 0:
            ...
            for step in possible_steps:
                ...
                if new_path.score <= shortest_distance_to[(step, new_path.direction)]:
                    shortest_distance_to[(step, new_path.direction)] = new_path.score
                else:
                    continue
                new_paths.append(new_path)
            current_path.add_to_path(step_for_the_current_path)
            if current_path.score <= shortest_distance_to[(step_for_the_current_path, current_path.direction)]:
                shortest_distance_to[(step_for_the_current_path, current_path.direction)] = current_path.score
            else:
                current_path.is_terminated = True
    paths.extend(new_paths)

best_paths = [path for path in paths if path.score == 75416]

all_fields_touched = set()
for best_path in best_paths:
    for element in best_path.path:
        all_fields_touched.add(element)

print(len(all_fields_touched))
