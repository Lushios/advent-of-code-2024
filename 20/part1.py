...
with open("input.txt") as file:
    origin_data = file.read().splitlines()
...
for i in range(len(origin_data)):
    for j in range(len(origin_data[i])):
        if origin_data[i][j] == 'S':
            start = (j, i)
        elif origin_data[i][j] == '#' and i != 0 and j != 0 and i != len(origin_data) - 1 and j != len(origin_data[i]) - 1:
            wall_positions.append((j, i))

print(len(wall_positions))
answers = 0
# 9308 is the number to beat
for wall in wall_positions:
    data = origin_data.copy()
    data[wall[1]] = origin_data[wall[1]][:wall[0]] + '.' + origin_data[wall[1]][wall[0] + 1:]
    ...
    if 9308 - min([len(path.path) for path in paths if path.is_finish]) - 1 >= 100:
        answers += 1
    print(min([len(path.path) for path in paths if path.is_finish]) - 1)
    print(wall)
print(answers)
# print(min([len(path.path) for path in paths if path.is_finish]) - 1)
